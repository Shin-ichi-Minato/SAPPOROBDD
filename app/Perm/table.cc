// Perm Table manipulation part (v0.1)
...
PiDD& FuncTable::GetPiDD(char* name) { return GetEntry(name) -> _p; }

void FuncTable::Set(char* name, PiDD p)
{
...
void FuncTable::Enlarge(void)
{
  ...
  for(int i=0; i<oldsize; i++)
    if(oldwheel[i]._live != 0)
      Set(oldwheel[i]._name, oldwheel[i]._p);
  delete[] oldwheel;
}
